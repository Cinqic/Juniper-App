...
**Version `0.3.0-rc.14` — standalone-runtime release candidate.** The release workflow builds and packages the Juniper-owned local runtime for desktop targets, then publishes install-smoked artifacts after the verification gates pass. See [Download](#download).

## Download

Installers will be published on the [GitHub releases page](https://github.com/Cinqic/Juniper-App/releases/tag/v0.3.0-rc.14) after the release workflow completes. You do not need Git, a build toolchain, or a GitHub account to use a published desktop artifact.

| Platform          | File                                        | Notes                                                             |
| ----------------- | ------------------------------------------- | ----------------------------------------------------------------- |
| Windows 10/11 x64 | `Juniper-0.3.0-rc.14-windows-x86_64.msi`    | The bundled local runtime is Juniper-owned; SmartScreen may warn. |
| Linux x86_64      | `Juniper-0.3.0-rc.14-linux-x86_64.AppImage` | `chmod +x`, then run. No installation required.                   |
| Linux x86_64      | `Juniper-0.3.0-rc.14-linux-x86_64.deb`      | `sudo apt install ./Juniper-...deb`                               |
| Android 7.0+      | `Juniper-0.3.0-rc.14-android-universal.apk` | Signed release APK; enable install from unknown sources.          |
...
- `0.3.0-rc.14` is a prerelease. It is published for evaluation and is not yet promoted to a final `0.3.0`.
