...
**Version `0.3.0-rc.10` — standalone-runtime release candidate.** The release workflow builds and packages the Juniper-owned local runtime for desktop targets, then publishes install-smoked artifacts after the verification gates pass. See [Download](#download).
...
| Android 7.0+      | `Juniper-0.3.0-rc.10-android-universal.apk` | Signed release APK; enable install from unknown sources.          |
...
- `0.3.0-rc.10` is a prerelease. It is published for evaluation and is not yet promoted to a final `0.3.0`.
